...
| | |
|---|---|
| **Coordinates** | `com.dkmads.ssp:ssp-android:0.5.4` |
...
```kotlin
dependencies {
  implementation("com.dkmads.ssp:ssp-android:0.5.4")
...
When DKMads provides `ssp-android-0.5.4.aar`, add it under `app/libs/`:

```kotlin
dependencies {
  implementation(files("libs/ssp-android-0.5.4.aar"))
}
```
