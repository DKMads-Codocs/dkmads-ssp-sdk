...
| | |
|---|---|
| **Coordinates** | `com.dkmads.ssp:ssp-android:0.5.2` |
...
```kotlin
dependencies {
  implementation("com.dkmads.ssp:ssp-android:0.5.2")
...
When DKMads provides `ssp-android-0.5.2.aar`, add it under `app/libs/`:

```kotlin
dependencies {
  implementation(files("libs/ssp-android-0.5.2.aar"))
}
```
