...
dependencies:
  dkmads_ssp:
    git:
      ...
      path: flutter
      ref: sdk-0.5.20   # release tag (sdk-<semver>)
```
...
Add `com.dkmads.ssp:ssp-android:0.5.20` per [Android Installation](./android.md).
...
Dashboard ad unit format must be **interstitial**. Uses IAB sizes (320×480 default), not screen pixels. **Pin native SDK `sdk-0.5.20`** — interstitial fit, click-through, and 90% letterbox chrome are implemented in native iOS/Android (Flutter inherits automatically).
